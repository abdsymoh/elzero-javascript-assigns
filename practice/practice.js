...
let swpCases = swappingCases.split("").map((ele) => (ele === ele.toUpperCase() ? ele.toLowerCase() : ele.toUpperCase())).join("");
...
let ignNumbers = ignoreNumbers.split("").map((ele) => isNaN(parseInt(ele)) ? ele : "").join("")
